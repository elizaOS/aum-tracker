  ...
  return html`
      ...
      <header class="px-6 lg:px-8 py-6 border-b border-glass-border">
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center space-x-4">
            <!-- Network Selector -->
            <div class="flex items-center space-x-2 px-4 py-2 glass-card rounded-lg">
              <div class="w-2 h-2 bg-green-400 rounded-full"></div>
              <span class="text-sm text-gray-300">All Networks</span>
            </div>
            
            <!-- Refresh Button -->
            <button onclick="refreshPortfolio()" class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 rounded-lg transition-colors">
        ...
        <div class="glass-card rounded-2xl p-8 mb-8 fade-in">
          <div class="flex items-center justify-between mb-8">
            ...
            <div class="flex items-center space-x-2">
              <button class="px-4 py-2 glass-card rounded-lg text-gray-300 hover:text-white transition-colors">
                Tokens
              </button>
              <button class="px-4 py-2 glass-card rounded-lg text-gray-300 hover:text-white transition-colors">
                NFTs
          ...
          <div class="px-6 py-4 border-b border-glass-border">
            <div class="flex items-center justify-between">
              <h3 class="text-lg font-semibold text-white">
                Assets 
                <span id="assets-count" class="text-sm font-normal text-gray-400 ml-2">(0)</span>
              </h3>
              <div class="relative">
                <input 
                  type="text" 
        ...
        <div class="mt-8 glass-card rounded-2xl overflow-hidden fade-in">
          <div class="px-6 py-4 border-b border-glass-border">
            <div class="flex items-center justify-between mb-4">
              ...
              <p class="text-sm text-gray-400"><span id="active-wallets">0</span> active</p>
            </div>
            <div class="relative">
              <input 
                type="text" 
    ...
    <script>
      ...
      async function updateDashboard(data) {
        ...
        // Update total value
        document.getElementById('total-value').textContent = formatCurrency(data.totalUSDValue);
        document.getElementById('sol-amount').textContent = formatNumber(data.totalSOL);
        ...
        // Update metrics
        document.getElementById('change-24h').textContent = formatPercentage(change24h);
        document.getElementById('change-24h').className = \`text-xl font-semibold \${getChangeColor(change24h)}\`;
        
        document.getElementById('change-7d').textContent = formatPercentage(data.priceChange7d || 0);
        document.getElementById('change-7d').className = \`text-xl font-semibold \${getChangeColor(data.priceChange7d || 0)}\`;
        
        document.getElementById('change-30d').textContent = formatPercentage(data.priceChange30d || 0);
        document.getElementById('change-30d').className = \`text-xl font-semibold \${getChangeColor(data.priceChange30d || 0)}\`;
        
        document.getElementById('total-wallets').textContent = data.totalWallets;
        document.getElementById('unique-tokens').textContent = data.uniqueTokens;
      ...
      function displayAssets(assets) {
        ...
        if (assets.length === 0) {
          tbody.innerHTML = '<tr><td colspan="5" class="px-6 py-8 text-center text-gray-400">No assets found</td></tr>';
          return;
        }
      ...
      function createAssetRow(asset) {
        ...
        row.className = 'border-b border-glass-border hover:bg-white/5 transition-colors cursor-pointer asset-row';
        row.setAttribute('data-searchable', (asset.symbol + ' ' + asset.name).toLowerCase());
      ...
      async function loadWallets() {
        try {
          ...
          if (result.success && result.data) {
            ...
            let activeCount = allWallets.filter(w => w.fetch_status === 'success').length;
      ...
      function displayWallets(wallets) {
        ...
        if (wallets.length === 0) {
          tbody.innerHTML = '<tr><td colspan="5" class="px-6 py-8 text-center text-gray-400">No wallets found</td></tr>';
          return;
        }
      ...
      window.filterWallets = function() {
        const searchTerm = document.getElementById('wallets-search').value.toLowerCase();
      ...
      window.refreshPortfolio = async function() {
        try {
          // First trigger price refresh
          await fetch('/api/tokens/prices/refresh', { method: 'POST' });
